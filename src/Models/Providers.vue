...
import { contextLimit, formatTokens } from './data'
...
const activeP = computed(
  () => byProvider.value.find((p) => p.id === active.value) || null,
)
    ...
    <aside class="prov-aside" aria-label="供应商列表">
      ...
      <div class="prov-list">
        <div
          ...
          :class="[
            'prov-item',
            { active: active === p.id, pinned: pinSet.has(p.id) },
          ]"
          role="button"
          tabindex="0"
        ...
        >
          ...
          <button
            type="button"
            :class="['prov-pin', { on: pinSet.has(p.id) }]"
            :title="pinSet.has(p.id) ? '取消置顶' : '置顶'"
            :aria-label="pinSet.has(p.id) ? '取消置顶' : '置顶'"
            :aria-pressed="pinSet.has(p.id)"
            @click="togglePin(p.id, $event)"
          >
    ...
    <div class="prov-main">
      <template v-if="activeP">
        ...
        <div class="prov-models" role="list">
          ...
          <div v-if="filteredModels.length === 0" class="list-empty">
            无匹配模型
          </div>
        </div>
      </template>
